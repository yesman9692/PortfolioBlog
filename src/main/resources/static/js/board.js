...
let visitIndex={
    init: function () {
        $("#btn-visit-save").on("click", ()=>{ //()={} 이걸 쓴 이유: this를 바인딩하기 위함.
            this.visitsave();
        });
        $("#visit-btn-delete").on("click", ()=>{ //()={} 이걸 쓴 이유: this를 바인딩하기 위함.
            this.visitDelete();
        });
        $("#btn-visit-update").on("click", ()=>{ //()={} 이걸 쓴 이유: this를 바인딩하기 위함.
            this.visitUpdate();
        });
    },
    ...
    visitsave: function () {
        ...
        $.ajax({ //요청
            ...
            alert("글쓰기가 완료되었습니다");
            location.href = "/board/visitBoard";
        }).fail(function(error){ //실패하면 fail실행
            alert(JSON.stringify(error));
        }); //ajax통신을 이용해서 3개의 데이터를 json으로 변경하여 insert요청
    },

    visitDelete: function () {
        var id = $("#id").text();
        // console.log("visitDelete: id=" + id);
        //ajax호출시 default가 비동기호출.
        $.ajax({ //요청
            type:"DELETE", //=insert
            url:"/api/visitBoard/"+id,
            dataType:"json" //응답이 json이라면, js오브젝트로 변경해줌
